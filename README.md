...
Мобильную платформу Frob можно как заказать напрямую у разработчиков, так и собрать самостоятельно. Для этого ниже предоставлены список необходимых для закупки и производства деталей, а так же инструкция по сборке.
#### Закупка необходимых деталей
Ниже вы найдете ссылку на таблицу для закупки необходимых компонентов. Цены на компоненты ориентировочны и могут меняться.
- [Список компонентов](https://github.com/dark516/Frob_robot/blob/main/assembly_guide/FROB_parts_and_prices.xlsx)
Помимо компонентов, перечисленных выше, вам так же будет необходимо самостоятельно произвести:
* Корпус робота (Ссылка на печатные детали)
* Плату питания (Ссылка на плату питания)
* Контроллер моторов (Ссылка на плату для моторов)
#### Сборка платформы
После приобретения всех деталей, вы можете приступить к сборке. Для этого следуйте пошаговой инструкцие, представленной ниже:

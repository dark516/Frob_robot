...
```
Заменив r1 на id вашей платформы. Или воспользоваться классическим методом подключения через ip аддрес. Вы можете посмотреть IP на раздающем сеть устройстве или установить плагин для сканирования всех IP-адресов:
```bash

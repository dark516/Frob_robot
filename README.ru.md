...
```
### Запуск робота
1. Подключите Arduino к Raspberry Pi через USB.
2. Выполните команду для просмотра списка подключенных устройств:  
```bash
   ls /dev/tty*
```
3. Найдите устройство, которое появилось после подключения Arduino. Обычно это будет что-то вроде ```/dev/ttyUSB0``` или ```/dev/ttyACM0```.
4. Используя найденный порт запустите большинство функциональных модулей, выполнив в рабочей директории на роботе:
```bash
ros2 launch frob_bringup bringup.launch.py arduino_port:=/dev/ttyUSB0
```
